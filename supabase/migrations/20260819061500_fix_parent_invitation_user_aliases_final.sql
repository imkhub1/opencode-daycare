...
as $$
  select exists (
    ...
    where app_users.id = (select auth.uid())
      ...
      and app_users.role in ('staff'::public.user_role, 'admin'::public.user_role)
...
begin
  ...
  join public.rooms as daycare_rooms on daycare_rooms.id = daycare_children.room_id
  where daycare_children.id = p_child_id
    and daycare_children.status = 'active'::public.child_status
  for update of daycare_children;

  if v_child_daycare_id is null or v_child_daycare_id <> v_caller_daycare_id then
    raise exception 'Unable to create parent invitation';
  end if;
  ...
  insert into public.invitations (
    child_id, invited_by, full_name, email, relationship, code_hash,
    code_ciphertext, status, delivery_status, expires_at
  )
  values (
    p_child_id, v_caller_id, btrim(p_full_name), v_normalized_email,
    p_relationship, p_code_hash, btrim(p_code_ciphertext),
...
revoke all on function public.create_parent_invitation(
  uuid, text, text, public.relationship_type, text, text
) from public, anon, authenticated;
...
begin
  ...
  return query
  select links.parent_id, app_users.full_name, auth_users.email,
         links.relationship, app_users.status::text
  from public.parent_children as links
  join public.users as app_users on app_users.id = links.parent_id
  join auth.users as auth_users on auth_users.id = links.parent_id
  where links.child_id = p_child_id
