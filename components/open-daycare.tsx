...
const navItems: { href: string; label: string; icon: IconName }[] = [{ href: "/", label: "Feed", icon: "home" }, { href: "/ninos", label: "Niños", icon: "users" }, { href: "/avisos", label: "Avisos", icon: "bell" }, { href: "/mi-cuenta", label: "Mi cuenta", icon: "user" }];

function NavLinks({ compact = false }: { compact?: boolean }) {
  return <nav className={compact ? "flex flex-col gap-1" : "flex flex-1 flex-col gap-1"}>{navItems.map((item) => <Link key={item.href} href={item.href} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold ${item.href === "/" ? "bg-coral-soft text-[#d9583c]" : "text-[#6e6359] hover:bg-[#f6ecdf]"}`}><Icon name={item.icon} className="size-5" />{item.label}</Link>)}</nav>;
}

export function Sidebar() {
  return <aside className="sticky top-0 hidden h-screen w-[248px] shrink-0 flex-col border-r border-line bg-surface px-4 py-6 md:flex"><div className="px-2 pb-6"><Brand /></div><Link href="/crear-publicacion" className="mb-5 flex items-center justify-center gap-2 rounded-[14px] bg-linear-to-b from-[#f4977e] to-[#ee8164] px-3 py-3 text-sm font-extrabold text-white shadow-lg shadow-[#ee8164]/25"><Icon name="plus" className="size-[17px]" />Nueva publicación</Link><NavLinks /><div className="mt-3 border-t border-line pt-4"><div className="flex items-center gap-3 px-2"><Avatar>C</Avatar><div className="min-w-0 flex-1"><p className="text-sm font-extrabold text-ink">Caro Giménez</p><p className="text-xs text-[#a89a8b]">Maestra · Soles</p></div><Link href="/cerrar-sesion" aria-label="Cerrar sesión" className="flex size-8 items-center justify-center rounded-lg bg-sand text-muted hover:text-coral"><Icon name="log-out" className="size-4" /></Link></div></div></aside>;
}

export function MobileNavigation() {
  return <header className="sticky top-0 z-10 border-b border-line bg-surface/95 px-4 py-3 backdrop-blur md:hidden"><div className="flex items-center justify-between"><Brand /><details className="relative"><summary className="flex size-10 cursor-pointer list-none items-center justify-center rounded-xl bg-sand text-ink"><Icon name="menu" className="size-5" /><span className="sr-only">Abrir navegación</span></summary><div className="absolute right-0 top-12 w-56 rounded-2xl border border-line bg-surface p-2 shadow-xl shadow-[#3f362e]/10"><NavLinks compact /></div></details></div></header>;
}

type FeedPost = { type: "LOGRO" | "ACTIVIDAD" | "ANUNCIO"; time: string; body: string; audience: string; reactions: number; comments: number; photo?: string };
const postStyle = { LOGRO: "bg-[#cfebd8] text-[#3e9b6c]", ACTIVIDAD: "bg-[#c7e7f1] text-[#2e89a6]", ANUNCIO: "bg-[#ccd8f4] text-[#4e72c8]" };
