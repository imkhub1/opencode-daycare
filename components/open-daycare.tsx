...
function NavLinks({ activeHref = "/", compact = false }: { activeHref?: string; compact?: boolean }) {
  return <nav aria-label="Navegación principal" className={compact ? "flex flex-col gap-1" : "flex flex-1 flex-col gap-1"}>{navItems.map((item) => {
    const isActive = item.href === activeHref;
    const link = <Link href={item.href} aria-current={isActive ? "page" : undefined} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold ${isActive ? "bg-coral-soft text-[#d9583c]" : "text-[#6e6359] hover:bg-[#f6ecdf]"}`}><Icon name={item.icon} className="size-5" />{item.label}</Link>;
